...
const Cart: NextPage = () => {
  ...
  const isSameSet = (s1: any, s2: any) => {
    const getComp = (v: any) => JSON.stringify([...v].sort());
  ...
  useEffect(() => {
    // console.log(uniqueStoreList);
    // console.log(uniqueStoreObjs);
    // console.log("useeffect")
    // console.log(cartData?.getCurrentUser?.carts.length);
    // if (cartsQty.length !== cartData?.getCurrentUser?.carts.length) {
    // if (cartData?.getCurrentUser?.carts.length < uniqueStoreList.length) {
    //   setUniqueStoreList([])
    // }
    // if (cartData?.getCurrentUser?.carts.length < uniqueStoreObjs.length) {
    //   setUniqueStoreObjs([])
    // }
    // cartData?.getCurrentUser?.carts.map((cart: any) => {
    //   if (uniqueStoreList.indexOf(cart?.product?.shop?.id) == -1) {
    //     setUniqueStoreList([...uniqueStoreList, cart?.product?.shop?.id]);
    //     setUniqueStoreObjs([...uniqueStoreObjs, cart?.product?.shop]);
    //   }
    // });
    // let values = [...cartsQty];
    // let currSet = new Set();
    // uniqueStoreObjs.map((shop: any) => {
    //   cartData?.getCurrentUser?.carts.map((cart: any, index: number) => {
    //     if (cart?.product?.shop?.id == shop?.id) {
    //       // let cart = cartsQty
    //       // cart.add({label: cart?.id, quantity: cart?.quantity})
    //       // console.log(index)
    //       currSet.add({ label: cart?.id, quantity: cart?.quantity })

    //       // values[index] = cartData?.getCurrentUser?.carts[index]?.quantity;

    //       // setCartsQty([...cartsQty, {label: cart?.id, quantity: cart?.quantity}])
    //     }
    //   });
    // });
    // console.log(currSet)
    // console.log("is Same Set: " + isSameSet(currSet, prevSet))
    // if (isSameSet(currSet, prevSet) === false) {
    //   // if (isSameSet(new Set(cartsQty), currSet) === false) {
    //     setCartsQty(Array.from(currSet))
    //   // } else if (isSameSet(currSet, prevSet) === false) {
    //     setPrevSet(currSet)
    //   // }
    // }
    // setPrevSet(currSet)
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>Keranjang | Tohopedia</title>
                ...
                .map((shop: any, index: number) => {
                  ...
                  return (
                    ...
                    >
                      {index > 0 ? (
                        <div
                          className={styles.cart_lists_store_separator}
                        ></div>
                      ) : null}
                      <div
                        className={styles.cart_lists_stores_details_container}
                      >
                        <div
                          className={
                        ...
                        <div className={styles.cart_lists_stores_name_city}>
                          <div
                            className={styles.cart_lists_store_name_container}
                          >
                            <div
                          ...
                          <div
                            className={styles.cart_lists_store_city_container}
                          >
                  ...
                  <div className={styles.cart_main_summary_container}>
                    ...
                    >
                      <button
                        ...
                        onClick={() => { cartSummary.quantity > 0 ? router.push('/cart/shipment') : null }}
